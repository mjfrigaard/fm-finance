...
To convert a daily value in cell `A1` to an annual value:
```excel
=A1 * 365
```

To convert a weekly value in cell `A1` to a monthly value:
```excel
=A1 * 4.33
```
:::
...
Assuming the initial amount is in cell `A1`, the annual rate (as a decimal, e.g., 0.07) is in `B1`, and the number of years is in `C1`:
...
Assuming the upfront cost is in cell `A1` and the monthly savings amount is in cell `B1`:
