...
Personal budget spreadsheets are a great way to start learning about math, programming, and data management. We won't dive too deep into these topics here, but if you're reading this book, it's likely you already have enough data (i.e., income, bills and expenses) to justify putting it in a spreadsheet. 

Consider the example bank statement below with two weeks of purchases. A real export from your bank would look much the same: a **date**, a **description**, a **category**, and an **amount** (negative for money going out, positive for money coming in).
...
Once your transactions sit in columns and rows, it's much easier to answer budgeting questions like: 
...
Bogle would approve: **simplicity wins.** A well-built spreadsheet beats most apps because it forces you to engage with your numbers. Below is a basic structure we could use for the `bank_statement` data above:
...
We'll use exactly this dataset in the [Tracking Spending](tracking_spending.qmd) chapter to compute those numbers.
...
co_box(color = "b", 
  ...
  fold = TRUE,
  size = "1.10",
  header = "Functions in R and Python", 
  contents = "
Each calculation below is also written in R and Python. Because you'll re-use these formulas constantly, we'll wrap each one in a **function**: a named, reusable formula you define once and call with whatever numbers you have. 
...
Anything after a `#` is a comment the computer ignores.")
```

The R code below uses the [`tidyverse` package](https://www.tidyverse.org/) (loaded once below) and the Python code uses [`numpy` library](https://numpy.org/) where vectors help.
