...
You can build your own budget using a spreadsheet or code (like the examples below). Applications like Microsoft Excel or Google Sheets offer tools and templates to customize each sheet/tab to fit your needs (search for “budget template”). Most personal finance blogs also offer a free downloadable classic envelope spreadsheet.
...
Consider the code examples below: they create a bank statement table, which shows two weeks of purchases. A real export from your bank would look much the same: a **date**, a **description**, a **category**, and an **amount** (negative for outgoing money, positive for incoming money).
...
4.  **Track one number above all**: the important number is your savings rate (which we will cover in [Savings](savings.qmd)). Everything else is detail.
