...
### How to approach expense categories {.unnumbered}

The math is the same, but the psychology differs. Fixed expenses reward ruthlessness—John Bogle's principle of low-cost simplicity[^bogle] applies here. Lock in the lowest rate possible (usually your housing cost, which Brian Preston prioritizes early in @sec-foop below), then automate and stop thinking about it. This creates room for error elsewhere, as Morgan Housel emphasizes: the less mental energy you waste on fixed costs, the more cushion you have to recover from unexpected swings.

Variable expenses demand a different discipline. Ramit Sethi's Conscious Spending Plan[^sethi-csp] teaches us that guilt-free spending on *categories* works better than line-by-line scrutiny. Instead of tracking every grocery receipt, set a monthly limit for groceries and let yourself spend freely within it. This is evidence-based: Ben Felix's research shows that behavioral compliance matters more than perfect optimization.[^felix-behavioral]

Periodic expenses are the outliers. Because they're lumpy and personal (one person budgets for pet care, another for annual car maintenance) you can't borrow someone else's number. The 1/12 rule (setting aside one-twelfth of the annual cost each month) transforms an invisible crisis into visible planning.

[^bogle]: Bogle advocated for keeping investment costs and financial friction as low as possible. This principle extends directly to fixed expenses: minimize them once through comparison shopping and negotiation, then automate to avoid repeated mental costs. See [Bogleheads](https://www.bogleheads.org/).

[^sethi-csp]: Sethi's Conscious Spending Plan divides spending into fixed costs, savings, guilt-free spending (wants), and investments. The key insight is that tracking categories rather than individual transactions reduces decision fatigue and improves compliance. Learn more at the [Conscious Spending Basics](https://www.iwillteachyoutoberich.com/conscious-spending-basics/) guide.

[^felix-behavioral]: Felix's evidence-based approach to investing and personal finance emphasizes that behavioral factors—consistency, habit, and guilt-free compliance—outweigh optimization. A budget you follow imperfectly beats one you abandon. See [PWL Capital research](https://pwlcapital.com/learn/).
...
%%| fig-align: center
%%{init: {'theme': 'neutral', 'themeVariables': { 'fontFamily': 'monospace', "fontSize":"14px", 'ganttNumberSectionStyles': 4}}}%%

flowchart LR

    Expenses(["Monthly<br>Expenses"])
    
    subgraph Fixed["<strong>FIXED EXPENSES</strong>"]
        Rent("Rent/Mortgage")
    ...
    subgraph Variable["<strong>VARIABLE EXPENSES</strong>"]
        Groceries("Groceries")
        Utilities("Utilities")
        Gas("Gas/Transport")
    ...
    subgraph Periodic["<strong>PERIODIC EXPENSES</strong>"]
    ...
    style Variable fill:#F5F2E8,color:#1C1C1E,stroke:#1C1C1E,stroke-width:2px
    style Groceries fill:#F5F2E8,color:#1C1C1E,stroke:#1C1C1E,stroke-width:2px
    style Utilities fill:#F5F2E8,color:#1C1C1E,stroke:#1C1C1E,stroke-width:2px
    style Gas fill:#F5F2E8,color:#1C1C1E,stroke:#1C1C1E,stroke-width:2px
    style Dining fill:#F5F2E8,color:#1C1C1E,stroke:#1C1C1E,stroke-width:2px
    
    style Periodic fill:#f44242,color:#F5F2E8,stroke:#1C1C1E,stroke-width:2px
    style Taxes fill:#f44242,color:#F5F2E8,stroke:#1C1C1E,stroke-width:2px
    style CarMaint fill:#f44242,color:#F5F2E8,stroke:#1C1C1E,stroke-width:2px
    style Gifts fill:#f44242,color:#F5F2E8,stroke:#1C1C1E,stroke-width:2px
    style Medical fill:#f44242,color:#F5F2E8,stroke:#1C1C1E,stroke-width:2px
...
```{mermaid}
%%| fig-cap: 'Build Your Budget: Three Phases'
%%| fig-align: center
%%{init: {'theme': 'neutral', 'themeVariables': { 'fontFamily': 'monospace', "fontSize":"16px", 'primaryColor':'#0e9aa7', 'primaryBorderColor':'#1C1C1E', 'secondBkgColor':'#f44242', 'tertiaryColor':'#48a56a'}}}%%

flowchart TD
    Start(["Start:<br>Build Your Budget"]) --> S1("<b>SETUP PHASE</b><br><br>1. Track Spending<br>2. Categorize Expenses<br>3. Calculate Savings Rate<br>4. Choose Framework")

    S1 --> I1("<b>IMPLEMENTATION PHASE</b><br><br>5. Set Up Sinking Funds<br>6. Automate Everything")

    I1 --> O1("<b>ONGOING PHASE</b><br><br>7. Weekly Review<br>Monthly Adjust<br>8. Quarterly Recalibrate")

    O1 --> End(["Result:<br>Intentional Budget"])

    style Start fill:#48a56a,color:#F5F2E8,stroke:#1C1C1E,stroke-width:2px
    style S1 fill:#0e9aa7,color:#F5F2E8,stroke:#1C1C1E,stroke-width:3px
    style I1 fill:#f44242,color:#F5F2E8,stroke:#1C1C1E,stroke-width:3px
    style O1 fill:#86ddcd,color:#1C1C1E,stroke:#1C1C1E,stroke-width:3px
    style End fill:#48a56a,color:#F5F2E8,stroke:#1C1C1E,stroke-width:2px
```

## Financial Order of Operations {#sec-foo-intro}

A budget tells you how much money is left over each month. But it doesn't tell you what to do with it. That's where **Brian Preston's Financial Order of Operations** (@sec-foop) comes in.

The FOO is a nine-step framework that answers: *In what order should I allocate this money?* It prioritizes ruthlessly. Each step must be funded before moving to the next. The upcoming chapters walk through steps 3 to 8 in depth; here's the complete sequence:

1. **Deductibles covered**: Cash on hand for insurance deductibles
2. **Employer match**: Capture free money from your 401(k) match
3. **High-interest debt**: Eliminate toxic-rate debt (credit cards, payday loans)
4. **Emergency reserves**: Build 3 to 6 months of essential expenses in cash
5. **Roth IRA / HSA**: Max tax-free retirement accounts
6. **Max-out retirement**: Push 401(k) to annual limits
7. **Hyperaccumulation**: Invest in taxable brokerage accounts (the stage where wealth really accelerates)
8. **Prepaid future expenses**: Save for known large costs (education, down payment)
9. **Low-interest debt**: Extra payments on mortgages and student loans

Your budget is the foundation. It shows you how much money you have to work with. The FOO is the map. It shows you where that money should go. The order matters because each step protects you from a specific financial risk. Don't invest aggressively (step 7) before you've built an emergency fund (step 4) or captured your employer match (step 2). The chapters ahead will show you why.
